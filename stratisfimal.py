...
def run_all_rome_lib(num_nodes, num_graphs, num_drawings, edge_length_minimization, seq_bend, timelimit, save, savefile=None, shuffle=False, target=None, subgraph_reduction=False):
    ...
    if target is not None and int(target) != 0:
        ...
        to_optimize = random.sample(os.listdir(f"datasets/Rome-Lib/graficon{num_nodes}nodi"), num_graphs)
    else:
        to_optimize = os.listdir(f"datasets/Rome-Lib/graficon{num_nodes}nodi")[:num_graphs]
    for file in to_optimize:
        ...
        if num_drawings > 0:
            num_drawings -= 1
            vis.draw_graph(g, f"datasets/Rome-Lib/{file}")
    if save:
...
def run_test_pos_1_to_n():
    n_nodes = 57
    to_optimize = os.listdir(f"datasets/Rome-Lib/graficon{n_nodes}nodi")[:10]
...
def run_my_algorithm():
    n_nodes = 77
    to_optimize = os.listdir(f"datasets/Rome-Lib/graficon{n_nodes}nodi")[:1]
    for to_opt in to_optimize:
        g = layering.create_better_layered_graph(f"graficon{n_nodes}nodi/{to_opt}", 4, 2)[0]
...
def run_standard_version():
    n_nodes = 68
    to_optimize = os.listdir(f"datasets/Rome-Lib/graficon{n_nodes}nodi")[0:1]
    for to_opt in to_optimize:
        optimizer = LayeredOptimizer(f"datasets/Rome-Lib/graficon{n_nodes}nodi/{to_opt}")
        optimizer.symmetry_breaking = True
        optimizer.optimize_layout()
...
def butterfly_experiment():
    ...
    opt68 = os.listdir(f"datasets/Rome-Lib/graficon68nodi")
    opt69 = os.listdir(f"datasets/Rome-Lib/graficon69nodi")
    to_optimize = [opt68[5], opt69[5], opt69[8], opt68[16], opt68[41], opt68[56]]
    # to_optimize = os.listdir(f"datasets/Rome-Lib/graficon{n_nodes}nodi")[50: 60]
    times1 = []
    optvals1 = []
    times2 = []
    optvals2 = []
...
def transitivity_experiment():
    n_nodes = 67
    to_optimize = os.listdir(f"datasets/Rome-Lib/graficon{n_nodes}nodi")[:20]
    times1 = []
    optvals1 = []
    times2 = []
    optvals2 = []
...
def randomly_select_files_for_exp(fname):
    ...
    for i, fold in enumerate(rome_folds):
        file = os.listdir(f"datasets/Rome-Lib/graficon{fold}nodi")[rome_files[i]]
        write_file_name(f"datasets/Rome-Lib/graficon{fold}nodi/{file}", fname)
...
def case_study_graph_experiment(command):
    ...
    junger_vals = []
    sankey_vals = []
    control_flow_file = f"datasets/control-flow-graphs/{command}/dbg.main.dot"
    for i in range(5):
