...
def gansner_init_ordering(gr: graph.LayeredGraph):
...
def gansner_wmedian(gr: graph.LayeredGraph, order, c_iter):
	for lnum, lay in enumerate(order):
		for nd in lay:
			mval = gansner_median_value(gr, nd, gr[nd].layer + 2*(c_iter % 2) - 1)  # y = 2x-1 maps 0 to -1 and 1 to 1
			gr[nd].y = mval if mval != -1 else gr[nd].y
		lay.sort(key=lambda x: gr[x].y)


def gansner_median_value(gr: graph.LayeredGraph, v, adj_rank):
	if adj_rank > gr[v].layer:
		p = [gr[nd].y for nd in gr.adj_list[v][1]]
		p.sort()
	elif adj_rank < gr[v].layer:
		p = [gr[nd].y for nd in gr.adj_list[v][0]]
		p.sort()
	if len(p) == 0:
		return -1
	elif len(p) % 2 == 1:
...
def gansner_transpose(gr: graph.LayeredGraph, rank, flat_rank):
	improved = True
	while improved:
		improved = False
...
def calc_if_swap_improves(rank, gr: graph.LayeredGraph, v, w):  # requirement: v is before w in the ranking
	n_cr_noswap = 0
	n_cr_swap = 0
	# indices = {v_adj: rank[gr[v].layer - 1].index(v_adj) for v_adj in gr.adj_list[v][0]}
	# indices.update({v_adj: rank[gr[v].layer + 1].index(v_adj) for v_adj in gr.adj_list[v][1]})
	# indices.update({w_adj: rank[gr[v].layer - 1].index(w_adj) for w_adj in gr.adj_list[w][0]})
	# indices.update({w_adj: rank[gr[v].layer + 1].index(w_adj) for w_adj in gr.adj_list[w][1]})
	for i in range(2):
		for v_adj in gr.adj_list[v][i]:
			for w_adj in gr.adj_list[w][i]:
				if v_adj != w_adj:
