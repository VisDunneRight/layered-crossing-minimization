...
def draw(g: graph.LayeredGraph, svg_name, node_x_distance=100, node_y_distance=100):
    ...
    for edge in g.edges:  # curve_to(c1x, c1y, c2x, c2y, ex, ey), control points c1, c2, end point e
        ...
        if edge.same_layer_edge:
            ...
            ctx.curve_to((edge.n1.layer - 1) * node_x_distance + offset + node_x_distance//1.5, edge.n1.y * node_y_distance + offset, (edge.n2.layer - 1) * node_x_distance + offset - node_x_distance//1.5, edge.n2.y * node_y_distance + offset, (edge.n2.layer - 1) * node_x_distance + offset, edge.n2.y * node_y_distance + offset)
        ctx.stroke()
